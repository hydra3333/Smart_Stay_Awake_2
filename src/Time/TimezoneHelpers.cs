// File: src/Smart_Stay_Awake_2/Time/TimezoneHelpers.cs
...
namespace Smart_Stay_Awake_2.Time
{
    ...
    internal static class TimezoneHelpers
    {
        ...
        {
            Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Entered ParseUntilTimestamp with: '{timestamp}'");
            ...
            if (!match.Success)
            {
                Trace.WriteLine("Smart_Stay_Awake_2: TimezoneHelpers: Regex match FAILED - invalid format");
            ...
            Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Parsed components: Y={year} M={month} D={day} H={hour} M={minute} S={second}");
            ...
            try
            {
                ...
                Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Calendar validation PASSED: {localDt:yyyy-MM-dd HH:mm:ss}");
            }
            catch (ArgumentOutOfRangeException ex)
            {
                Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Calendar validation FAILED: {ex.Message}");
            ...
            Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Local timezone: {localZone.Id} (StandardName={localZone.StandardName})");

            // Check if this local time is invalid (spring-forward gap)
            if (localZone.IsInvalidTime(localDt))
            {
                Trace.WriteLine("Smart_Stay_Awake_2: TimezoneHelpers: DST validation FAILED - nonexistent time (spring-forward gap)");
            ...
            {
                Trace.WriteLine("Smart_Stay_Awake_2: TimezoneHelpers: DST validation FAILED - ambiguous time (fall-back overlap)");
                throw new ArgumentException(
                    "--until is ambiguous (falls in the repeated DST fall-back hour). " +
                    "Please choose a different time.");
            }

            Trace.WriteLine("Smart_Stay_Awake_2: TimezoneHelpers: DST validation PASSED (no spring-forward gap, no fall-back ambiguity)");

            // Step 4: Convert to UTC (guaranteed safe now)
            DateTime utcDt = TimeZoneInfo.ConvertTimeToUtc(localDt, localZone);
            Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Converted to UTC: {utcDt:yyyy-MM-dd HH:mm:ss} UTC");

            // Step 5: Convert to epoch seconds
            epochSeconds = (utcDt - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds;
            Trace.WriteLine($"Smart_Stay_Awake_2: TimezoneHelpers: Epoch seconds: {epochSeconds:F1}");

            Trace.WriteLine("Smart_Stay_Awake_2: TimezoneHelpers: Exiting ParseUntilTimestamp (success)");
